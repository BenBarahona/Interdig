...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    VOIPCallViewController *app = delegate.voipVC;
    //  NSNumber *value;
...
pj_status_t sip_startup(app_config_t *app_config)
{
    ...
    if (val < 0 || val > 65535)
    {
        ...
        NSLog(@"Invalid value for Local Port (expecting 1-65535)");
    ...
    if (app_config->rtp_cfg.port < 1 || app_config->rtp_cfg.port > 65535)
    {
        ...
        NSLog(@"Invalid value for RTP port (expecting 1-65535)");
        status = PJ_EINVAL;
        goto error;
    }
...
pj_status_t sip_connect(pj_pool_t *pool, pjsua_acc_id *acc_id)
{
    ...
    if ((status = pjsua_verify_sip_url(acc_cfg.id.ptr)) != 0)
    {
        ...
        NSLog(@"Invalid value for username or server");
        return status;
    }
    ...
    if (acc_cfg.reg_timeout < 1 || acc_cfg.reg_timeout > 3600)
    {
        ...
        NSLog(@"Invalid value for timeout (expecting 1-3600)");
        return PJ_EINVAL;
    }
    ...
    while (anObject = [enumerator nextObject])
    {
        ...
        if (proxy && strlen(proxy))
        {
            ...
            if ((status = pjsua_verify_sip_url(pj_proxy.ptr)) != 0)
            {
                ...
                NSLog(@"Invalid value for proxy parameter");
                continue;
            }
    ...
    *acc_id = aid;
    ...
    if (status != PJ_SUCCESS)
    {
        ...
        NSLog(@"Error adding new account");
    }
#endif
    return status;
}
...
static pj_status_t sip_add_account(NSDictionary *account, pjsua_acc_id *acc_id)
{
    pj_status_t status;
    pjsua_acc_config acc_cfg;
    const char *uname;
    ...
    if ((status = pjsua_verify_sip_url(acc_cfg.reg_uri.ptr)) != 0) 
    {
        PJ_LOG(1,(THIS_FILE,  "Error: invalid SIP URL '%s' in registrar argument",
                  acc_cfg.reg_uri));
        //[app displayParameterError: @"Invalid value for server parameter."];
        NSLog(@"Invalid value for server parameter");
        return status;
    }
